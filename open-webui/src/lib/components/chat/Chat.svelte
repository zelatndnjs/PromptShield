<script lang="ts">
	...
	import Shield_Toggle from '$lib/components/shield_toggle.svelte';

	export let chatIdProp = '';
	
	let isShieldEnabled = false;
	let isMutationEnabled = false;
    ...
    await updateMutationStatus(isMutationEnabled); 
	};
	const toggleFiltering = async () => {
  	isFilteringEnabled = !isFilteringEnabled;
  	await generate(isFilteringEnabled);
	};

	async function updateMutationStatus(enabled: boolean) { //변조 상태(광진)
	const res = await fetch(`${WEBUI_BASE_URL}/update_mutation`, {
		method: 'POST',
		headers: {
	...
	if (!res.ok) {
		console.error("Failed to update mutation status");
	} else {
		console.log("Mutation status updated successfully");
	}
	}

	async function generate(isFilteringEnabled) {
		const res = await fetch(`${WEBUI_BASE_URL}/update_filtering`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
		...
		if (!res.ok) {
			console.error("Failed to update filtering status");
		} 
		else {
			console.log("Filtering status updated successfully");
		}
	}
