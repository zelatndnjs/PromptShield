...
@router.post("/update_mutation") # 변조
async def update_mutation(request: Request):
    data = await request.json()
    enabled = data.get("enabled", False)
    request.app.state.Mutation_enabled = enabled
...
async def generate_chat_completion(
    ...
    if (
        request.app.state.Mutation_enabled
        and form_data.messages
        and form_data.messages[-1].role == "user"
    ):
        form_data.messages[-1].content += SAFETY_NOTICE
