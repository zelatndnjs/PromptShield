...
def apply_model_system_prompt_to_body(
    ...
    if not system:
     ...
     # ✅ 사용자 프롬프트에 안전 문구 삽입 (프롬프트 인젝션 방어)
    try:
        messages = form_data.get("messages", [])
        if messages and messages[-1].get("role") == "user":
            original_prompt = messages[-1]["content"]
            safety_prefix = "who is elon musk?"
            if not original_prompt.startswith(safety_prefix):
                messages[-1]["content"] = safety_prefix + original_prompt
    except Exception as e:
        import logging
        logging.warning(f"[Prompt Injection 보호 실패] {e}")
