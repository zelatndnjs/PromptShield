...
#비윤리적 단어 조합(3)
    (r"(ip|아이피).{0,10}(우회|숨기기|변조).{0,10}(툴|도구|방법)", "IP 우회 도구 조합"),
    (r"(백도어|루트킷).{0,10}(설치|삽입|사용).{0,10}(방법|절차|툴)", "백도어 설치 기법"),
    (r"(vpn|프록시).{0,10}(우회|익명).{0,10}(접속|활용)", "VPN 프록시 우회 접속 조합"),
    (r"(디도스|ddos).{0,10}(공격|시뮬레이션).{0,10}(툴|테스트)", "DDoS 공격 도구"),
    (r"(시리얼|크랙|키젠).{0,10}(툴|프로그램).{0,10}(다운로드|공유)", "크랙툴 유통 조합"),
    (r"(필로폰|대마|마약).{0,10}(구매|판매|거래).{0,10}(사이트|주소|경로)", "마약 거래 경로 조합"),
    (r"(향정신성|불법).{0,10}(약물|알약).{0,10}(복용|오남용|처방)", "불법 약물 복용 조합"),
    (r"(마약).{0,10}(합성|제조).{0,10}(레시피|과정|도구)", "마약 합성 과정 조합"),
    (r"(미성년자|아동).{0,10}(성관계|성착취).{0,10}(영상|유출|공유)", "미성년 대상 성범죄 조합"),
    (r"(불법|몰래).{0,10}(촬영|녹화).{0,10}(카메라|장비|앱)", "불법 촬영 장비 조합"),
    (r"(몸캠|야동).{0,10}(피싱|유도).{0,10}(갈취|협박|요구)", "몸캠 협박 조합"),
    (r"(불법|사설).{0,10}(도박|카지노).{0,10}(사이트|주소|링크)", "불법 도박 사이트 조합"),
    (r"(주식|코인).{0,10}(작전|선동).{0,10}(단톡방|채널|오픈채팅)", "시세 조작 커뮤니티 조합"),
    (r"(보이스피싱|스미싱).{0,10}(계좌|입금).{0,10}(요구|갈취|사기)", "보이스피싱 사기 조합"),
    (r"(폭탄|화약|폭발물).{0,10}(제조|조립).{0,10}(방법|도구|재료)", "폭발물 제조 조합"),
    (r"(총기|칼|흉기).{0,10}(판매|구매).{0,10}(사이트|주소|방법)", "불법 무기 거래 조합"),
    (r"(살인|테러).{0,10}(계획|실행).{0,10}(방법|수단|무기)", "살상 행위 실행 조합"),
    (r"(개인정보|계정).{0,10}(유출|수집).{0,10}(툴|사이트|방법)", "개인정보 수집 조합"),
    (r"(사이트|서비스).{0,10}(관리자|루트).{0,10}(권한|탈취|우회)", "관리자 권한 탈취 조합"),
    (r"(비밀번호|패스워드).{0,10}(찾기|크랙).{0,10}(툴|공격|방식)", "비밀번호 크랙 조합"),
    (r"(내란|폭동).{0,10}(선동|계획).{0,10}(온라인|메신저|단톡)", "사회 혼란 조장 조합"),
    (r"(테러|극단주의).{0,10}(홍보|포섭).{0,10}(사이트|포럼|영상)", "극단주의 확산 조합"),
    (r"(해킹).{0,10}(강의|기법).{0,10}(유출|공유|자료)", "해킹 강의 유출 조합"),
    (r"(스파이웨어|키로거).{0,10}(설치|배포).{0,10}(방법|툴|사이트)", "스파이웨어 배포 조합"),
    (r"(디스코드|텔레그램).{0,10}(익명|비공개).{0,10}(공유방|자료방)", "익명 공유방 조합"),
    (r"(디프웹|다크웹).{0,10}(접속|이용).{0,10}(방법|도구|주소)", "다크웹 접근 조합"),
    (r"(통장|계좌).{0,10}(대여|판매).{0,10}(사이트|방법|거래)", "대포통장 거래 조합"),
    (r"(운전면허증|신분증).{0,10}(위조|복제).{0,10}(앱|프로그램|도구)", "신분증 위조 조합"),
    (r"(학생|청소년).{0,10}(조건|알바).{0,10}(유도|모집|광고)", "청소년 조건 유도 조합"),
    (r"(카카오톡|카톡).{0,10}(사칭|피싱).{0,10}(링크|계정|사이트)", "카카오 피싱 사기 조합"),
    (r"(n번방|박사방).{0,10}(영상|자료).{0,10}(구매|유통|공유)", "n번방 유출물 조합"),
    (r"(에셋|암호화폐).{0,10}(세탁|환전).{0,10}(방법|사이트|도구)", "암호화폐 자금세탁 조합"),
    (r"(도박|베팅).{0,10}(승률|조작).{0,10}(공식|코드|AI)", "도박 승률 조작 조합"),
    (r"(성인|야동).{0,10}(리벤지|복수).{0,10}(영상|사진|공개)", "리벤지포르노 유포 조합"),
    (r"(해외).{0,10}(원정|출장).{0,10}(성매매|조건만남|알선)", "해외 성매매 알선 조합"),
    (r"(불법|크랙).{0,10}(소프트웨어|os).{0,10}(다운로드|공유|설치)", "불법 소프트웨어 유통 조합"),
    (r"(인증서|토큰).{0,10}(위조|탈취).{0,10}(로그인|접속|사이트)", "전자 인증서 위조 조합"),
    (r"(페이팔|계좌).{0,10}(털기|해킹).{0,10}(방법|툴|강의)", "금융 해킹 조합"),
    (r"(아르바이트|구직자).{0,10}(유도|갈취).{0,10}(계좌|신분증|사진)", "구직자 착취 조합"),
    (r"(환각제|향정신성).{0,10}(복용|흡입).{0,10}(영상|후기|리뷰)", "환각제 복용 조합"),
    (r"(성폭행|강간).{0,10}(미화|농담).{0,10}(커뮤니티|짤|자료)", "성범죄 미화 조합"),
    (r"(몰카|불법촬영).{0,10}(유포|유통).{0,10}(링크|사이트|주소)", "몰카 유포 조합"),
    (r"(학교|대학).{0,10}(폭력|따돌림).{0,10}(영상|유출|공유)", "학교폭력 영상 유포 조합"),
    (r"(성인용|야설).{0,10}(웹툰|만화).{0,10}(사이트|주소|공유)", "불법 성인 웹툰 유통 조합"),
    (r"(워터마킹|저작권).{0,10}(제거|우회).{0,10}(툴|방법|앱)", "저작권 우회 도구 조합"),
    (r"(게임).{0,10}(핵|치트).{0,10}(다운로드|툴|공유)", "게임 핵 치트 공유 조합"),
    (r"(자동|매크로).{0,10}(사냥|채굴).{0,10}(프로그램|앱|툴)", "게임 불법 매크로 조합"),
    (r"(신용카드|체크카드).{0,10}(정보|번호).{0,10}(판매|거래|공유)", "카드 정보 유출 조합"),
    (r"(지문|홍채).{0,10}(복제|탈취).{0,10}(장비|기술|툴)", "생체정보 탈취 조합"),
    (r"(유흥|오피스텔).{0,10}(조건만남|성매매).{0,10}(알선|광고|소개)", "조건만남 유흥 알선 조합"),
    (r"(불법|미인가).{0,10}(스포츠중계|영상).{0,10}(사이트|주소|공유)", "불법 스포츠중계 조합"),
    (r"(무기|폭발물).{0,10}(구매|수입).{0,10}(경로|사이트|주소)", "무기 밀매 경로 조합"),
    (r"(신분세탁|계좌세탁).{0,10}(방법|정보).{0,10}(강의|자료|사이트)", "신분세탁 정보 조합"),
    (r"(유서|극단적).{0,10}(선택|행동).{0,10}(도움|방법|공유)", "극단적 선택 유도 조합"),
    (r"(임산부|임신부).{0,10}(낙태|약물).{0,10}(방법|정보|도구)", "비의료적 낙태 조합"),
    (r"(보안|감시).{0,10}(우회|회피).{0,10}(방법|툴|프로그램)", "보안 회피 도구 조합"),
    (r"(도청|감청).{0,10}(기기|앱).{0,10}(설치|구매|사용)", "도청 장비 사용 조합"),
    (r"(실시간|라이브).{0,10}(성인|불법).{0,10}(방송|사이트|영상)", "불법 라이브 방송 조합"),
    (r"(무단|불법).{0,10}(개조|해체).{0,10}(기기|차량|드론)", "불법 개조 장비 조합"),
    (r"(보험|보상).{0,10}(사기|조작).{0,10}(수법|방법|툴)", "보험 사기 기법 조합"),
    (r"(청소년|미성년자).{0,10}(유해|불법).{0,10}(광고|유도|노출)", "청소년 유해노출 조합"),
]
